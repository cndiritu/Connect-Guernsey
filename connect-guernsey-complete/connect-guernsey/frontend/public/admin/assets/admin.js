...
let _token = localStorage.getItem('cg_token');
...
function requireAuth() {
  if (!_token) { location.href = '/admin/index.html'; return false; }
  return true;
}
...
function buildSidebar(activePage) {
  ...
  sidebar.innerHTML = `
    ...
    <nav class="sidebar-nav">
      ...
      <a href="/admin/enquiries.html" class="nav-item ${activePage==='enquiries'?'active':''}"><span class="icon">✉️</span> Enquiries <span class="nav-badge" id="unreadBadge" style="display:none">0</span></a>
  ...
  // Load unread count
  adminApi.get('/api/enquiries/').then(r => {
    const badge = document.getElementById('unreadBadge');
    if (badge && r && r.unread > 0) { badge.textContent = r.unread; badge.style.display = 'inline-block'; }
  }).catch(()=>{});
}
  ...
  container.innerHTML = `
    <div class="rich-editor">
      <div class="editor-toolbar">
        ...
        <button class="et-btn" onclick="execCmd('insertOrderedList')">1. List</button>
        <button class="et-btn" onclick="execCmd('formatBlock','h2')">H2</button>
        <button class="et-btn" onclick="execCmd('formatBlock','h3')">H3</button>
...
async function uploadFile(file) {
  ...
  fd.append('file', file);
  const r = await adminApi.upload('/api/media/upload', fd);
  return r.url;
}
